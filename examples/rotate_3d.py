import marimo

__generated_with = "0.23.0"
app = marimo.App(width="columns")

with app.setup:
    import manim as mn
    from manim import BLUE, Circle, Create, GREEN, OUT, RED, RIGHT, Rotate, UP
    from manim_widget import ManimWidget


@app.class_definition
class Rotate3D(ManimWidget):
    def construct(self):
        c1 = Circle(radius=1.5, color=RED, fill_opacity=0.8)
        c2 = Circle(radius=1.5, color=GREEN, fill_opacity=0.5)
        c2.rotate(mn.PI / 2, axis=RIGHT)
        c3 = Circle(radius=1.5, color=BLUE, fill_opacity=0.5)
        c3.rotate(mn.PI / 2, axis=UP)

        self.play(Create(c2), Create(c1), Create(c3))
        self.play(Rotate(c1, mn.PI, axis=UP, run_time=2))
        self.play(Rotate(c2, mn.PI, axis=RIGHT, run_time=2))
        self.play(Rotate(c3, mn.PI, axis=OUT, run_time=2))
        self.move_camera(phi=1.0, theta=0.8, run_time=2)


@app.cell
def _():
    Rotate3D()
    return


def test(runner):
    runner.check(Rotate3D).assert_ok()


if __name__ == "__main__":
    app.run()
